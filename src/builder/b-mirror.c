...
static const char * const git_helper_sh =
	...
	"    HASH=$3\n"
	"    MIRROR_PATH=$4\n"
	"    while ! mkdir \"$MIRROR_PATH.lock\" 2>/dev/null; do\n"
	...
	"    fi\n"
	"    if [ \"$REF\" != \"$HASH\" ]; then\n"
	"        git -C \"$MIRROR_PATH\" fetch \"$REMOTE_URL\" \"$REF\"\n"
	"    fi\n"
	"    git -C \"$MIRROR_PATH\" fetch --no-tags \"$REMOTE_URL\" \"$HASH\"\n"
	"    git -C \"$MIRROR_PATH\" update-ref \"ref-$HASH\" \"$HASH\"\n"
